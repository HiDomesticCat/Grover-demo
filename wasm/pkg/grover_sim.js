/* @ts-self-types="./grover_sim.d.ts" */

/**
 * ⌊(π/4)·√(N/M)⌋ for the UI.
 * @param {number} num_qubits
 * @param {number} num_targets
 * @returns {number}
 */
export function optimal_iterations_for(num_qubits, num_targets) {
    const ret = wasm.optimal_iterations_for(num_qubits, num_targets);
    return ret >>> 0;
}
 ...
 * @returns {any}
 */
export function simulate(num_qubits, target_indices, iterations, noise_value, shots, exact, noise_scale, seed) {
    const ptr0 = passArray32ToWasm0(target_indices, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.simulate(num_qubits, ptr0, len0, iterations, noise_value, shots, exact, noise_scale, seed);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('grover_sim_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
